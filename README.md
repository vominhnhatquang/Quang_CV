# CV Builder Output

Generated compact CV package for Võ Minh Nhật Quang.

## Files

- `outputs/Vo_Minh_Nhat_Quang_CV_Balanced.pdf` — balanced one-page PDF version with circular profile photo.
- `outputs/Vo_Minh_Nhat_Quang_CV_Balanced.docx` — editable Microsoft Word version matching the balanced PDF layout.
- `outputs/Vo_Minh_Nhat_Quang_CV.md` — source Markdown version.
- `website/` — static CV website, including downloadable copies under `website/downloads/`.
- `assets/profile_from_previous_cv_1_1.png` — profile image extracted from the previous CV PDF.
- `assets/profile_circle.png` — circular/framed profile image used in current outputs.
- `data/resume_profile.json` — structured resume profile used to render the CV.
- `.github/workflows/pages.yml` — GitHub Actions workflow for GitHub Pages deployment.

## Deploy website

1. Push this repository to GitHub.
2. In repository settings, enable GitHub Pages with **GitHub Actions** as the source.
3. Push to `main` or run the `Deploy static CV site` workflow manually.

Private repositories are described in the CV without public links; public GitHub/Hugging Face artifacts are linked.
